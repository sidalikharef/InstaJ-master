...
public class GetInstagramProfils {

    Utils utils = new Utils();
    String elements;
	String[] tags;
    ArrayList<String> links = new ArrayList<>(); 
    int numberoffMessages;
    WebDriver driver;
  
   
    public GetInstagramProfils(WebDriver driver, TextArea tagsList, TextField numberOfMessages, TextArea logbox, TextField timeBetweenMessages) throws InterruptedException {
    	elements=tagsList.getText();
    	tags = elements.split("\\s*[,]\\s*");
        this.numberoffMessages = Integer.parseInt(numberOfMessages.getText());
        this.driver = driver;
        startLinksThread(logbox, timeBetweenMessages);
    } 
    ...
    private void getLinks(TextArea logbox) throws InterruptedException {
        ...
        int numberOfRows = (int) Math.ceil(((float) numberoffMessages / 3));
       /* if (numberOfLikes > 12) {
            searchTag(tags);
    ...
    public void startLinksThread(TextArea logbox, TextField timeBetweenmessages) throws InterruptedException {
        Task<Void> downloaderTask = linksTask(logbox);

        downloaderTask.setOnSucceeded((WorkerStateEvent t) -> { 
            try {
            	SendMeSSages sendmessages = new SendMeSSages(links, timeBetweenmessages, logbox, driver);
            } catch (InterruptedException ex) {
                Logger.getLogger(GetInstagramProfils.class.getName()).log(Level.SEVERE, null, ex);
            }
